# Apache Guacamole Installation Script for RHEL 7 and CentOS 7
## Notice (1/31/2019)
...
- In typical scnearios, once the script completes the Guacamole server should be fully ready to use
...
- Ability to install a custom Guacamole extension, like my Guacamole Customize Login Page extension to change the appearence of the login page.
- LDAP/LDAPS authentication via Guacamole LDAP extension and configuration prompts.
- All SELinux conexts properly set instead of disabling SEL.
...
- `wget` installed to download the Guacamole istall script `guac-install.sh` from this repo.
...
- No prior Guacamole installation or configuration including for its major dependant packages like Nginx, Tomcat, mariaDB, etc.
...
## Apache Guacamole Intsall Script Information
...
As expressed on the offical site for Apache Guacamole (which is free and open source):
